# 9. Создание интерфейсов типа VLAN на fw-cod для маршрутизации между VLAN
...
Для маршрутизации трафика между VLAN в офисе COD необходимо создать на fw-cod (Ideco NGFW) интерфейсы типа VLAN. Межсетевой экран будет выполнять роль шлюза по умолчанию для всех VLAN.

| VLAN | Название | Подсеть | Шлюз (fw-cod) |
|------|----------|---------|---------------|
...
> ⚠️ **Примечание:** Для VLAN 200 (DATA) создавать интерфейс нет необходимости, т.к. по условиям задания трафик данного VLAN не должен маршрутизироваться.
...
|----------|----------|----------|
| Название | fw1-cod | Произвольное имя интерфейса |
| Роль | LAN | Роль интерфейса в сети |
| Зона | — | Поле необязательное |
| VCE | Системный контекст | Виртуальный контекст |
| Физический порт | eth1_da | Порт, направленный в сторону sw1-cod |
| Режим | Без конфигурации | IP-адреса назначаются на VLAN-интерфейсы |

> 📝 **Примечание:** Режим **«Без конфигурации»** выбран потому, что IP-адреса будут назначаться на интерфейсы VLAN, созданные на базе данного интерфейса.
...
### sw1-cod — Временная конфигурация (для проверки)
